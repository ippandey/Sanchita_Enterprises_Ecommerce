...
const FAQSection = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      <h2 className="text-3xl sm:text-5xl font-cormorant text-center mb-5">
        FAQs
      </h2>
      <p className="text-center font-work text-gray-600 mb-10 border-b pb-5">
        Have a question? We are here to help.
      ...
      <div className="space-y-4">
        {faqs.map((faq, index) => (
          <div key={index} className="border-b border-gray-200 pb-4">
            <button
              onClick={() => toggleFAQ(index)}
              className="w-full flex justify-between items-center text-left text-lg sm:text-xl font-cormorant font-medium text-gray-800 focus:outline-none"
            >
              <span>{faq.question}</span>
              <span className="text-2xl">
                {activeIndex === index ? "−" : "+"}
