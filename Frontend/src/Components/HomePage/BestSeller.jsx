    ...
    <section className="overflow-hidden relative">
      <div className="px-4 sm:px-10 md:px-20 py-10">
        ...
        >
          ...
          <motion.h1
            className="text-2xl sm:text-3xl md:text-4xl font-cormorant capitalize text-[#202025]"
            variants={{
              hidden: { opacity: 0, y: 50 },
              visible: {
                opacity: 1,
        ...
        {/* Products carousel */}
        <motion.div
          className="flex lg:grid lg:grid-cols-3 xl:grid-cols-4 gap-6 overflow-x-auto scrollbar-hide"
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, amount: 0.2 }}
        ...
        >
          {BestSellerList.map((product) => (
            <Link
              key={product._id}
              to={`/product/${product._id}`}
              className="min-w-[80%] sm:min-w-[45%] md:min-w-[30%] lg:min-w-0"
            >
              <AnimatePresence>
                <motion.div
                  className="border p-4 shadow-md rounded-lg bg-[#f7f4f3] relative hover:shadow-lg transition duration-300"
                  onMouseEnter={() => setHoveredProductId(product._id)}
                  onMouseLeave={() => setHoveredProductId(null)}
                  variants={{
                    hidden: { opacity: 0, y: 50 },
                ...
                >
                  ...
                  {(hoveredProductId === product._id ||
                    ...
                    <button className="absolute top-44 left-1/2 transform -translate-x-1/2 w-4/5 bg-white text-[#333333] hover:bg-[#333333] hover:text-white px-4 py-2 rounded-sm shadow-md transition duration-300">
                      Add to Cart
                    </button>
                  )}
                  ...
                  <img
                    ...
                    alt={product.title}
                    className="w-full h-52 object-cover mb-4 rounded-md transition-all"
                  />
                  <h3 className="text-lg font-medium text-center font-work text-[#333333]">
                    {product.title}
                  </h3>
                  ...
                  <p className="text-[#666666] text-base flex justify-center gap-2 mb-4">
                    {Array.from({ length: 5 }).map((_, index) =>
                      index < product.ratings ? (
                        <FaStar key={index} className="text-[#333333]" />
                      ) : (
                        <FaRegStar key={index} className="text-[#999999]" />
                      )
                    )}
                  </p>
